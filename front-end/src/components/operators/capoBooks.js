...
export default function Capobooks() {
    ...
    async function handleClick(bookID) {
        if (upgradedAvailability !== undefined) {
            await fetch(`http://localhost:9103/updateBookAvailability/${bookID}/${upgradedAvailability}`, {
                method: 'Post',
        ...
        if (upgradedImage !== undefined) {
            ...
            await fetch(`http://localhost:9103/updateBookImage/${bookID}/${i}`, {
                method: 'Post',
                headers: { 'Content-Type': 'application/json' }
            }).catch((e) => { console.log(e.message) })
        }
        if (upgradedPages !== undefined) {
            await fetch(`http://localhost:9103/updateBookPages/${bookID}/${upgradedPages}`, {
                method: 'Post',
                headers: { 'Content-Type': 'application/json' }
            }).catch((e) => { console.log(e.message) })
        }
        if (upgradedTitle !== undefined) {
            await fetch(`http://localhost:9103/updateBookTitle/${bookID}/${upgradedTitle}`, {
                method: 'Post',
                headers: { 'Content-Type': 'application/json' }
            }).catch((e) => { console.log(e.message) })
        }
        if (upgradedLanguage !== undefined) {
            await fetch(`http://localhost:9103/updateBookLanguage/${bookID}/${upgradedLanguage}`, {
                method: 'Post',
                headers: { 'Content-Type': 'application/json' }
            }).catch((e) => { console.log(e.message) })
        }
        if (upgradedSummary !== undefined) {
            await fetch(`http://localhost:9103/updateBookSummary/${bookID}/${upgradedSummary}`, {
                method: 'Post',
                headers: { 'Content-Type': 'application/json' }
            }).catch((e) => { console.log(e.message) })
        }
    ...
    return (
        <div>
            {!addbook && !query && !updatebook && <div className="container" style={{ position: "relative", top: "150px" }}>
                <h3>what would you like to do:</h3>
                <button onClick={() => { setAddbook(true); setUpdatebook(false) }} className="button">add a book</button>
                <button onClick={() => { setUpdatebook(true); setAddbook(false) }} className="button">update a book</button>
                <button onClick={() => { setQuery(true) }} className="button">view books</button>
            </div>}
            {addbook &&
                <div>
                        ...
                        <div>
                            {books?.map((book) => (
                                <div key={book.bookID} className="container" style={{ borderStyle: "dotted", borderColor: "purple" }}>
                                    ...
                                    <button onClick={handleClick.bind(null, book.bookID)} className="button">submit</button>
                                </div>
                            ))}
                        </div>
                    ) : (
