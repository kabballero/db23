...
export default function Kyles() {
    ...
    const handleChangeValues = (newvalue1, newvalue2, newvalue3, newvalue5) => {
        ...
        <div>
            ...
            {categories?.length > 0 ? (
                <div className="container">
                    ...
                    {submited && !seeborrowed && !seeprofile && !seereservations && <Books nameIN={name} category={select} title={title} />}
                    {seeborrowed && !seeprofile && !submited && !seereservations && <Borrowed />}
                    {!seeborrowed && seeprofile && !submited && !seereservations && <Profile />}
                    {!seeborrowed && !seeprofile && !submited && seereservations && <Reserve />}
                </div>) : (
                <h1>wait</h1>
            )}
        </div>
